...
class FindRandomGoal_Bot:
    ...
    def __init__(self, game, args, view_radius, controlled_by_env=False):
        if not args.no_monsters:
            raise ValueError(
                f"Bot '{self.__class__.__name__}' can't be used with monsters, please provide '--no-monsters' option"
            )
        if not args.harmless_barrels:
            raise ValueError(
                f"Bot '{self.__class__.__name__}' can't be used with explosive barrels, please provide '--harmless-barrels' option"
            )
    ...
    def reset(self, *, seed=None):
        ...
        if self.controlled_by_env:
            ...
            key = (
                ring.RingEntryType.RING_ENTRY_KEY_NOOP
            ...
            # Start new game
            key = (
                ring.RingEntryType.RING_ENTRY_KEY_NEW
                | ring.RingEntryType.RING_ENTRY_F_SINGLE_TICK_PRESS
            )
        ...
        env_whole = diablo_state.get_environment(
        ...
        env_empty = (
            ...
            | (
                env_whole
                == (
                    diablo_state.EnvironmentFlag.Explored.value
            ...
            (
                (
                    env_whole
                    & (
                        diablo_state.EnvironmentFlag.Door.value
    ...
    def step(self) -> tuple[bool, int]:
        ...
        def visibility_fn(p):
            ...
            # Visibility check
            return (
                1 if (env_local[p] & diablo_state.EnvironmentFlag.Visible.value) else 0
            )

        # Get cloud of visible tiles and frontier
        visible, frontier = maze.visibility_frontier(player_pos_local, visibility_fn)
        ...
        for lr, lc, gr, gc in zip(
            ...
            # flag explicitly
            if tile & diablo_state.EnvironmentFlag.Door.value or not (
        ...
        if (
            ...
            (
                in_local_bounds(goal_pos_local, env_local.shape)
                and env_local[goal_pos_local]
                & diablo_state.EnvironmentFlag.Visible.value
            )
        ):
            ...
            if not self.unexplored_points:
                ...
                self.save_state(
                    d, player_pos, next_unexplored, offset, env_local, visible, frontier
                )
                assert 0, "Nothing to explore"

            manhattan_dist = lambda p: np.abs(p[0] - player_pos[0]) + np.abs(
                p[1] - player_pos[1]
            )
        ...
        #
        if (
            player_pos not in self.current_path
            or next_unexplored not in self.current_path
        ):
        ...
        closed_door = lambda tile: (
            tile & diablo_state.EnvironmentFlag.Door.value
            and not (tile & diablo_state.EnvironmentFlag.Open.value)
        )
        ...
        if (
            (
                diag_move
                and (
                    barrel_or_item(env_local[r][nc]) or barrel_or_item(env_local[nr][c])
                )
            )
            or closed_door(next_tile)
            or barrel_or_item(next_tile)
        ):
        ...
        if not self.controlled_by_env:
            key = (
                diablo_env.DiabloEnv.action_to_key(action)
                | ring.RingEntryType.RING_ENTRY_F_SINGLE_TICK_PRESS
            )
    ...
    def save_state(
        self, d, player_pos, next_unexplored, offset, env_local, visible, frontier
    ):
        ...
        with open(f"{self.state_dir}/{self.steps_cnt}-vision.txt", "w") as f:
            ...
            print(
                f"visible_explored {[(int(t[0]),int(t[1])) for t in vis_explored]}",
                file=f,
            )
            ...
            for p_local in fr:
                ...
                if p in self.unexplored_points:
                    up.append(p)
            print(
                f"unexplored {[(int(t[0]),int(t[1])) for t in self.unexplored_points]}",
                file=f,
            )
            print(
                f"unexplored in frontier {[(int(t[0]),int(t[1])) for t in up]}", file=f
            )
...
class ExploreLevel_Bot:
    """
    An autonomous bot that explores a dungeon environment until
    all enemies are killed and items are looted
    ...
    def __init__(self, game, args, view_radius, controlled_by_env=True):
        if not args.harmless_barrels:
            raise ValueError(
                f"Bot '{self.__class__.__name__}' can't be used with explosive barrels, please provide '--harmless-barrels' option"
            )
    ...
    def reset(self, *, seed=None, d=None):
        if seed is not None:
            self.seed = seed
            self.rng = np.random.default_rng(seed)
            ...
            (
                (
                    ...
                    & (
                        ...
                        | diablo_state.EnvironmentFlag.Barrel.value
                        | diablo_state.EnvironmentFlag.Item.value
                        | diablo_state.EnvironmentFlag.Monster.value
                    )
                    != 0
                )
            )
        ...
        self.env_whole = env_whole
        self.env_empty = env_empty
        goal_positions = np.argwhere(
            env_whole == diablo_state.EnvironmentFlag.NextTrigger.value
        )
        if goal_positions.size > 0:
            self.goal_pos = tuple(goal_positions[0])
        else:
            self.goal_pos = None
        ...
        self.unexplored_points = set()
        self.current_path = []
        self.current_player_level = d.player.plrlevel
        self.steps_cnt = 0
        self.routes_rebuilds = 0
        self.discard_run = False

    def step(self, env_local, d) -> tuple[bool, int]:
        """Executes one reasoning-and-action cycle of the agent.
        ...
        """
        # d = self.game.safe_state
        if d.player.plrlevel > self.current_player_level:
            self.current_player_level += 1
            print(f"dungeon explored, good job!")
            return True, diablo_env.ActionEnum.Stand.value

        # end episode and do not save aything
        # if len(self.action_history) >= 1_000:
        #    if all(x == self.action_history[0] for x in self.action_history) or all(
        #        self.action_history[i] != self.action_history[i - 1] for i in range(1, len(self.action_history))
        #    ):
        ...
        # walls that have bars can be seen through, but we still want them to cound as unexplored, and thus we can
        ...
        unexplored_changed = False
        monsters_remaining = diablo_state.monsters_remaining(d)
        ...
        # sometimes we can find the goal locally, but not globally
        if self.goal_pos is None:
            x_bound, y_bound = env_local.shape
            for i in range(x_bound):
                for j in range(y_bound):
                    tile = env_local[(i, j)]
                    if tile & diablo_state.EnvironmentFlag.NextTrigger.value:
                        self.goal_pos = (i + offset[0], j + offset[1])
            # print(f"no goal found?, good job!")
            # return True, diablo_env.ActionEnum.Stand.value

        monster_positions = []
        breakable_items = []
        for i in range(env_local.shape[0]):
            for j in range(env_local.shape[1]):
                local_tuple = (i, j)
                if env_local[local_tuple] & diablo_state.EnvironmentFlag.Monster.value:
                    monster_positions.append(local_tuple)
                if env_local[local_tuple] & (diablo_state.EnvironmentFlag.Barrel.value):
                    breakable_items.append(local_tuple)

        # Select next unexplored destination - either none, or the
        # last point in the current path
        next_unexplored = None if not self.current_path else self.current_path[-1]

        # sort the monster locations beginning with the closest
        if monster_positions:
            dist_fn = lambda m: abs(player_pos_local[0] - m[0]) + abs(
                player_pos_local[1] - m[1]
            )
            nearest_monsters = sorted(monster_positions, key=dist_fn)
            nearest_monsters_global = [
                (m[0] + offset[0].item(), m[1] + offset[1].item())
                for m in nearest_monsters
            ]

            nearest_monster = nearest_monsters_global[0]
            # only build a new path if none of the current monsters are on the path
            all_monsters_on_path = np.all(
                [m in self.current_path for m in nearest_monsters_global]
            )
            if not all_monsters_on_path or player_pos not in self.current_path:
                ...
                def cost_fn(p):
                    ...
                    return (
                        ...
                        if (
                            self.env_empty[p]
                            or p == nearest_monster
                            or p == next_unexplored
                        )
                        else 0
                    )

                path = maze.shortest_path(player_pos, nearest_monster, cost_fn)
                if not path:
                    self.save_state(
                        d,
                        player_pos,
                ...
                self.current_path = path
                # if the player faces the wrong direction, we need to append the current path again, because
                # we need an additional action before we can attack
                if len(self.current_path) == 1:
                    self.current_path.append(self.current_path[0])

        # if there are no monsters, keep exploring
        elif (len(monster_positions) == 0 and monsters_remaining > 0) or (
            player_pos not in self.current_path
            or next_unexplored not in self.current_path
        ):
            # Check frontiers
            ...
            for lr, lc, gr, gc in zip(
                ...
                if self.explored[gpt]:
                    continue
                tile = env_local[(lr, lc)]
                # The closed door has a `wall` flag, so check the
                # flag explicitly
                if tile & diablo_state.EnvironmentFlag.Door.value or not (
                    tile & diablo_state.EnvironmentFlag.Wall.value
                ):
                    if gpt not in self.unexplored_points:
                        self.unexplored_points.add(gpt)
                        unexplored_changed = True
                        self.last_unexplored_changed = gpt

            manhattan_dist = lambda p: np.abs(p[0] - player_pos[0]) + np.abs(
                p[1] - player_pos[1]
            )

            if self.unexplored_points:
                next_unexplored = min(self.unexplored_points, key=manhattan_dist)
            # all monsters are dead and we can go to the goal
            else:
                next_unexplored = self.goal_pos
            ...
            if (
                ...
                path = maze.shortest_path(player_pos, next_unexplored, cost_fn)
                if not path:
                    # self.save_state(d, player_pos, next_unexplored, offset, env_local, visible, frontier)
                    self.discard_run = True
                    return True, np.zeros(())
                    # assert 0, "Path is empty"
            ...
            if (
                self.goal_pos not in self.current_path
                or player_pos not in self.current_path
            ):
                ...
                def cost_fn(p):
                    ...
                    # (e.g. solid objects), but we need to build a path to
                    # these objects anyway.
                    return 1 if (self.env_empty[p] or p == self.goal_pos) else 0

                path = maze.shortest_path(player_pos, self.goal_pos, cost_fn)
                if not path:
                    self.discard_run = True
                    return True, np.zeros(())
                self.current_path = path
        ...
        # Select the next point
        step_point = (
            self.current_path[1] if len(self.current_path) > 1 else self.current_path[0]
        )
        step_point_local = tuple(step_point - offset)
        move_dir = tuple(np.array(step_point_local) - np.array(player_pos_local))
        r, c = player_pos_local
        ...
        def direction_to_action(direction):
            ...
            if direction == (-1, 1):
                ...
                return diablo_env.ActionEnum.Stand.value
        ...
        # There was an issue where the door was closed and the monster was inside the door
        attackble = lambda tile: tile & (diablo_state.EnvironmentFlag.Monster.value)

        next_tile = env_local[step_point_local]
        action = 0
        # Always attack monsters first.
        if attackble(next_tile):
            action = diablo_env.ActionEnum.PrimaryAction.value

        # elif all([a == diablo_env.ActionEnum.SecondaryAction.value for a in self.action_history]):
        #    action = direction_to_action(move_dir)
