...
structref.define_proxy(
    DevilutionState, DevilutionStateType, [var["short_name"] for var in dx.VARS]
)
...
def is_door_open(obj):
    return (
        obj._oVar4 == DoorState.DOOR_OPEN.value
        or obj._oVar4 == DoorState.DOOR_BLOCKED.value
    )
...
@njit(cache=True)
def is_barrel(obj):
    return obj._otype in (
        dx._object_id.OBJ_BARREL.value,
...
def is_chest(obj):
    return obj._otype in (
        dx._object_id.OBJ_CHEST1.value,
...
def is_sarcophagus(obj):
    return obj._otype in (dx._object_id.OBJ_SARC.value, dx._object_id.OBJ_L5SARC.value)


@njit(cache=True)
def is_floor(d, pos):
    return not (
        d.SOLData[d.dPiece[pos]]
...
@njit(cache=True)
def monsters_remaining(d):
    env_whole = get_environment(d, show_invisible=True, show_unexplored=True)
    return np.sum(env_whole == EnvironmentFlag.Monster.value)
...
@njit(cache=True)
def get_environment(
    d, radius=None, goal_pos=None, show_invisible=False, show_unexplored=False
):
    ...
    # (height, weight)
    env = np.zeros((env_rect.drect.width, env_rect.drect.height), dtype=np.uint32)
...
def pick_random_empty_tile_pos(env: np.ndarray, rng: np.random.Generator):
    ...
    empty_env = (
        ...
        | (env == (EnvironmentFlag.Explored.value | EnvironmentFlag.Visible.value))
    )
...
def get_dungeon_graph_and_path(env, start, goal):
    ...
    empty_env = (env & (EnvironmentFlag.Wall.value | EnvironmentFlag.Door.value)) == 0
    ...
    regions_graph, regions_doors, doors_matrix = maze.get_regions_graph(
    ...
    regions_path = maze.bfs_regions_path(regions_graph, start_region, goal_region)
...
def map_devilutionx_state(path, offset):
    ...
    for var in dx.VARS:
        ...
        obj_array = np.frombuffer(
            mmapped, dtype=dtype, count=1, offset=var_offset
        ).view(np.recarray)
        obj = obj_array[0]
...
def map_devilutionx_state_by_pid(pid, mshared_path):
    for attempt in range(0, 50):
        try:
            ...
            _, offset = procutils.get_mapped_file_and_offset_of_pid(pid, mshared_path)
            if not offset:
                # Wait until remapped
                time.sleep(0.1)
                continue
...
def movement_to_true_north(keys):
    ...
    if movement_keys == (ring.RingEntryType.RING_ENTRY_KEY_UP):
        ...
        ring.RingEntryType.RING_ENTRY_KEY_UP | ring.RingEntryType.RING_ENTRY_KEY_RIGHT
    ):
        # NE
        ...
        ring.RingEntryType.RING_ENTRY_KEY_DOWN | ring.RingEntryType.RING_ENTRY_KEY_RIGHT
    ):
        # SE
        ...
        ring.RingEntryType.RING_ENTRY_KEY_DOWN | ring.RingEntryType.RING_ENTRY_KEY_LEFT
    ):
        # SW
        ...
        ring.RingEntryType.RING_ENTRY_KEY_UP | ring.RingEntryType.RING_ENTRY_KEY_LEFT
    ):
        # NW
...
class DiabloGame:
    ...
    def run(config):
        ...
        cfg = cfg.format(
            ...
            enemies_deal_no_damage=1 if config["enemies-deal-no-damage"] else 0,
            disable_loot=1 if config["disable-loot"] else 0,
        )
